...
class AppServiceProvider extends ServiceProvider
{
    ...
    {
        if (config('filafluxkit.admin_panel_enabled', false)) {
            $this->app->register(AdminPanelProvider::class);
        }
        if (config('filafluxkit.app_panel_enabled', false)) {
            $this->app->register(AppPanelProvider::class);
        }
        if (config('filafluxkit.guest_panel_enabled', false)) {
            $this->app->register(GuestPanelProvider::class);
        }
        if (config('filafluxkit.favicon.enabled')) {
    ...
    private function configureSchema(): void
    {
        Schemas\Schema::configureUsing(function (Schemas\Schema $schema) {
            return $schema
                ->defaultCurrency(config('filafluxkit.defaultCurrency'))
                ->defaultDateDisplayFormat(config('filafluxkit.defaultDateDisplayFormat'))
                ->defaultIsoDateDisplayFormat(config('filafluxkit.defaultIsoDateDisplayFormat'))
                ->defaultDateTimeDisplayFormat(config('filafluxkit.defaultDateTimeDisplayFormat'))
                ->defaultIsoDateTimeDisplayFormat(config('filafluxkit.defaultIsoDateTimeDisplayFormat'))
                ->defaultNumberLocale(config('filafluxkit.defaultNumberLocale'))
                ->defaultTimeDisplayFormat(config('filafluxkit.defaultTimeDisplayFormat'))
                ->defaultIsoTimeDisplayFormat(config('filafluxkit.defaultIsoTimeDisplayFormat'));
        });
    }

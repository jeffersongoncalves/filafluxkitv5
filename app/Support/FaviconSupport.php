...
abstract class FaviconSupport
{
    ...
    {
        $manifest = config('filafluxkit.favicon.manifest');
...
<browserconfig>
    ...
    {
        return response(Vite::content(config('filafluxkit.logo')), 200, ['Content-Type' => 'image/png']);
    }

    private static function getFavicon(): Response
    {
        return response(Vite::content(config('filafluxkit.favicon')), 200, ['Content-Type' => 'image/x-icon']);
    }
}
